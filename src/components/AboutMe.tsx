  ...
  return (
    <section id="about" className="py-24 px-6 bg-background/95 backdrop-blur-sm">
      <div className="container mx-auto max-w-6xl">
        ...
        {/* Adjusted Grid Layout for the entire About Me section content */}
        {/* We'll use a 3-column grid for md screens and up:
            Column 1: DegreeProgress (narrow)
            Column 2: Profile Picture / Social (wider)
            Column 3: About Me Text (widest)
        */}
        {/* Adjusted the first column width to accommodate the rotated progress bar */}
        <div className="grid grid-cols-1 md:grid-cols-[120px_2fr_3fr] gap-12 items-start">

          {/* Degree Progress Bar (Left Sidebar placement) */}
          {/* Removed md:pt-16, as vertical centering is handled within DegreeProgress now */}
          <div className="col-span-1 flex flex-col items-center md:items-start animate-fade-in">
             <DegreeProgress />
          </div>

          {/* Left Column - Profile (now center column on md screens) */}
          ...
          {/* Right Column - Bio (now the rightmost column on md screens) */}
