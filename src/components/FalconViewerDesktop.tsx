...
import { Search, MousePointer2, AlertCircle, ArrowDown, ChevronRight, ChevronDown, X, CornerUpLeft, Hand, Rotate3d } from "lucide-react";
...
const ZOOM_ZONES: Record<string, ZoomZone> = {
  ...
  fairing: { offset: [132, 41, -211], lookOffset: [-100, 20, -50], type: "dynamic", refId: "top" },
  "second stage booster": { offset: [70, 20, 100], lookOffset: [0, 30, 0], type: "dynamic", refId: "top" },
  interstage: { offset: [214, 127, 13], lookOffset: [-50, 120, 0], type: "dynamic", refId: "middle" },
...
const PART_DETAILS: Record<string, { title: string; subtitle: string; description: string; specs: { label: string; value: string }[]; features: string[] }> = {
  fairing: {
    ...
    specs: [{ label: "Height", value: "13.1 m / 43 ft" }, { label: "Diameter", value: "5.2 m / 17 ft" }, { label: "Mass", value: "~1900 kg" }, { label: "Material", value: "Carbon Composite" }],
    features: ["Pneumatic separation system", "Recoverable via sea vessels", "Acoustic absorption blankets", "Radio-transparent windows"]
  },
  "second stage booster": {
    title: "Second Stage", subtitle: "Orbital Insertion Stage",
    ...
    specs: [{ label: "Engine", value: "1x Merlin Vacuum" }, { label: "Thrust", value: "981 kN / 220.5 klbf" }, { label: "Burn Time", value: "397 sec" }, { label: "Propellant", value: "LOX / RP-1" }],
    features: ["Multiple restart capability", "Redundant flight computers", "Nitrogen gas thrusters for attitude control", "High-strength Al-Li alloy tanks"]
  },
  interstage: {
    title: "Interstage", subtitle: "Stage Connection & Separation",
    ...
    specs: [{ label: "Length", value: "Estimated ~8m" }, { label: "Material", value: "Carbon Fiber Composite" }, { label: "System", value: "Pneumatic Pusher" }],
    features: ["Encloses Merlin Vacuum Engine", "Protects engine nozzle during ascent", "Houses Grid Fin actuators", "Grid fin stowage points"]
  },
  gridfins: {
    title: "Titanium Grid Fins", subtitle: "Hypersonic Control Surfaces",
    ...
    specs: [{ label: "Material", value: "Cast Titanium" }, { label: "Actuation", value: "Open Hydraulic System" }, { label: "Quantity", value: "4 Fins" }],
    features: ["Withstands reentry temperatures", "Foldable for ascent aerodynamics", "Precise landing site targeting", "Passive thermal protection"]
  },
  "merlin 9 boosters": {
    title: "First Stage / Octaweb", subtitle: "Main Propulsion System",
    ...
    specs: [{ label: "Engines", value: "9x Merlin 1D" }, { label: "Thrust (SL)", value: "7,607 kN / 1.7M lbf" }, { label: "Thrust (Vac)", value: "8,227 kN / 1.8M lbf" }, { label: "TWR", value: "> 150:1 (Engine)" }],
    features: ["Octaweb stress distribution structure", "Deep throttling capability", "Propulsive landing capability", "Engine-out redundancy"]
  },
};
...
function ZoomIndicator({ controlsRef }: { controlsRef: any }) {
  ...
  useFrame(() => {
    ...
    setZoomPct(Math.round((BASE_DIST / dist) * 100));
  });
...
function CameraCollision({ controlsRef, partsRefs }: { controlsRef: any, partsRefs: any }) {
  ...
  useFrame(() => {
    if (!controlsRef.current) return;
    const parts = [partsRefs.top.current, partsRefs.middle.current, partsRefs.bottom.current];
...
function InstructionOverlay({ onDismiss }: { onDismiss: () => void }) {
  return (
    <div onClick={onDismiss} className="absolute inset-0 z-[200] bg-neutral-950/60 backdrop-blur-[2px] flex flex-col items-center justify-center cursor-pointer transition-all duration-500 hover:bg-neutral-950/50 group px-4">
      <div className="flex flex-col items-center animate-in fade-in zoom-in duration-500">
        ...
        <h2 className="text-4xl font-black text-white tracking-tighter uppercase drop-shadow-lg mb-2 text-center">Tap to Explore</h2>
  ...
  return (
    <div className="border-b border-white/10 last:border-0">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full py-4 flex items-center justify-between text-left group">
        <span className="text-sm font-bold uppercase tracking-wider text-neutral-200 group-hover:text-white transition-colors">{title}</span>
        {isOpen ? <ChevronDown className="w-4 h-4 text-neutral-400" /> : <ChevronRight className="w-4 h-4 text-neutral-400" />}
      </button>
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-96 opacity-100 mb-4' : 'max-h-0 opacity-0'}`}>{children}</div>
    </div>
  );
}

export default function FalconViewerDesktop() {
  ...
  return (
    <div className="w-full h-[600px] lg:h-[750px] relative bg-white border border-neutral-200 overflow-hidden shadow-sm group font-sans">
      <style>{`
        .custom-scrollbar::-webkit-scrollbar { width: 8px; }
        .custom-scrollbar::-webkit-scrollbar-track { background: transparent; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(80, 80, 80, 0.5); border-radius: 20px; border: 2px solid rgba(0,0,0,0); background-clip: content-box; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(120, 120, 120, 0.8); border: 2px solid rgba(0,0,0,0); background-clip: content-box; }
      `}</style>
      ...
      <div className={`absolute top-8 left-8 z-50 transition-all duration-500 ${isOverview ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
        <h1 className="text-3xl lg:text-4xl font-black text-neutral-900 tracking-tighter">FALCON 9</h1>
      ...
      {/* PART SELECTION - Right side vertical */}
      <div className={`absolute z-40 transition-all duration-500 right-8 top-1/2 -translate-y-1/2 ${isOverview ? 'opacity-100' : 'opacity-0 pointer-events-none translate-x-20'}`}>
        <div className="absolute -top-16 right-0 w-48 text-right flex flex-col items-end gap-1 animate-bounce">
          <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest bg-white/90 px-2 py-1 rounded">Click On Each Part To View In More Detail!</span>
        ...
        <div className="flex flex-col gap-2">
          {Object.keys(ZOOM_ZONES).map((zone) => {
            ...
            return (
              <button key={zone} onClick={() => handleZoneClick(zone)} className="text-right text-[10px] font-bold tracking-wider uppercase px-4 py-2 rounded-md transition-all duration-200 bg-white/95 text-neutral-600 hover:text-neutral-900 hover:bg-white hover:scale-105 border border-neutral-200 hover:border-neutral-300 shadow-sm">
      ...
      {/* SIDEBAR */}
      <div className={`absolute z-50 top-0 left-0 h-full w-[400px] text-white shadow-2xl transition-transform duration-700 ease-bezier flex flex-col bg-neutral-950/95 backdrop-blur-xl ${!isOverview ? 'translate-x-0' : '-translate-x-full'}`} style={{ transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)' }}>
        <div className="p-6 lg:p-8 pb-2 shrink-0 border-b border-white/5 relative z-20 bg-neutral-950/50 backdrop-blur-md">
          <div className="flex justify-between items-center gap-2 mb-3">
            <button onClick={() => setCurrentZone("overview")} className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 rounded-lg transition-all border border-red-500/20 group">
              <CornerUpLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
              <span className="text-xs font-bold uppercase tracking-widest">Back</span>
            </button>
            <div className="px-2 py-1 border border-blue-500/30 bg-blue-500/10 rounded-full text-[10px] font-bold tracking-[0.15em] uppercase text-blue-400">System Analysis</div>
            <button onClick={() => setCurrentZone("overview")} className="p-1 rounded-full hover:bg-white/10 transition-colors text-neutral-500 hover:text-white">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-8 pt-2 relative z-10 pb-20">
          {currentDetails && (
            <>
              <h2 className="text-3xl lg:text-4xl font-black tracking-tighter leading-none mb-2 text-transparent bg-clip-text bg-gradient-to-br from-white to-neutral-500">{currentDetails.title}</h2>
              <h3 className="text-base lg:text-lg text-blue-400 font-medium mb-6 tracking-tight flex items-center gap-2">{currentDetails.subtitle}</h3>
              <div className="w-full h-px bg-white/10 mb-6" />
              <p className="text-neutral-300 leading-relaxed text-sm font-light opacity-90 mb-8 border-l-2 border-blue-500 pl-4">{currentDetails.description}</p>
      ...
      {/* SLIDER - Desktop position */}
      <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center gap-3 w-80 lg:w-96 p-4 rounded-2xl border shadow-xl backdrop-blur-md transition-all duration-500 ${!isOverview ? 'opacity-0 pointer-events-none translate-y-20' : 'opacity-100 translate-y-0'} ${warning ? 'bg-red-50/90 border-red-200 ring-2 ring-red-400 ring-offset-2' : 'bg-white/95 border-neutral-200'}`}>
        ...
        <input type="range" min="0" max="1" step="0.01" value={exploded} onChange={(e) => setExploded(parseFloat(e.target.value))} className={`w-full h-2 rounded-lg appearance-none cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-neutral-400 ${warning ? 'bg-red-200 accent-red-500' : 'bg-neutral-200 accent-neutral-900 hover:accent-neutral-700'}`} />
      </div>
      ...
      <Canvas shadows dpr={[1, 2]} camera={{ fov: 45, position: [300, 50, 400], far: 5000 }}>
        ...
        <Suspense fallback={<Loader />}>
          ...
          <CameraControls ref={cameraControlsRef} minPolarAngle={0} maxPolarAngle={Math.PI / 1.6} minDistance={80} maxDistance={600} />
